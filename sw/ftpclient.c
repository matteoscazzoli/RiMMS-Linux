...
static int 
build_cmd(char *buf,unsigned bufsize,char *cmd,char *arg1)
{
  int cnt;
...
/* Read one line from the server, being careful not to overrun the
   ...
   the line. */
static int get_line(int s, char *buf, unsigned buf_size, ftp_printf_t ftp_printf) 
{
  int eol = 0;
  int cnt = 0;
  int ret;
  char c;
  int timeout,waitdata;
  
  
  waitdata=0;
  while((!eol)&&(waitdata<100)) {
    timeout=0;
	do{
		ret = read(s,&c,1);
		usleep(1000);
	}while((ret<=0)&&(timeout++<5000));
		
	if(timeout>=5000){
	  ftp_printf(1,"read timeout %d\n",timeout);
      return FTP_BAD;
	}
    if (ret != 1) {
      ftp_printf(1,"read %s\n",strerror(errno));
...
static int get_reply(int s, ftp_printf_t ftp_printf) 
{
  char buf[BUFSIZ];
  int more = 0;
  int ret;
  ...
  do{
    ...
    if (first_line) {
      ...
      if (isdigit(buf[0]) && isdigit(buf[1]) && isdigit(buf[2]) && (code == strtoul(buf,NULL,0)) &&  buf[3]==' ') {
        more=0;
      } else {
        more =1;
      }
  ...
  //ftp_printf(0,"FTP_server_reply: %u\n",(buf[0] - '0'));
  return (buf[0] - '0');
}

/* Send a command to the server */
static int send_cmd(int s, char * msgbuf, ftp_printf_t ftp_printf) 
{  
  int len;
...
static int command(char * cmd, char * arg, int s, char *msgbuf, int msgbuflen, ftp_printf_t ftp_printf) 
{
  int err;
...
static int connect_to_server(char *hostname, struct sockaddr * local, char *portftp, ftp_printf_t ftp_printf)
{
	int sockfd = 0, n = 0;
    ...
    {
        ftp_printf(1,"Error : Could not create socket \n");
	...
	*/
    ...
    serv_addr.sin_port = htons(atoi(portftp)); 
	
	if (setsockopt (sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,sizeof(timeout)) < 0){
        ftp_printf(1,"setsockopt RCV failed\n");
		return FTP_NOSUCHHOST;
	}

    if (setsockopt (sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout,sizeof(timeout)) < 0){
        ftp_printf(1,"setsockopt SND failed\n");
		return FTP_NOSUCHHOST;
	}
	
	if(hostname_to_ip(hostname,ip)!=0){
		ftp_printf(1,"Error resolving address\n");
		return FTP_NOSUCHHOST;
	}
	
	if(inet_pton(AF_INET, ip, &serv_addr.sin_addr)<=0)
    {
        ftp_printf(1,"inet_pton error occured\n");
        return FTP_NOSUCHHOST;
    } 
	
	if( connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0)
    {
       ftp_printf(1,"Error : Connect to %s failed \n",ip);
       return FTP_NOSUCHHOST;
    } 
	...
	getnameinfo((struct sockaddr *)&serv_addr, sizeof(serv_addr), name, sizeof(name), port, sizeof(port), NI_NUMERICHOST|NI_NUMERICSERV);
    ftp_printf(0,"FTP2: Connected to %s:%s\n", name, port);
...
static int login(char * username, char *passwd, int s, char *msgbuf, unsigned msgbuflen, ftp_printf_t ftp_printf) 
{
  int ret;
...
static int opendatasock(int ctrl_s, struct sockaddr *ctrl, char *msgbuf, unsigned msgbuflen, ftp_printf_t ftp_printf) 
{
	...
	int iMode = 0;
	ioctl(ctrl->sa_family, FIONBIO, &iMode);
	/*flags = fcntl(ctrl->sa_family, F_GETFL);
    flags |= O_NONBLOCK;
    fcntl(ctrl->sa_family, F_SETFL, flags);
	*/
	
	if (setsockopt (s, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,sizeof(timeout)) < 0){
        ftp_printf(1,"setsockopt DATA failed%s\n",strerror(errno));
		return FTP_BAD;
	}
    ...
    if (bind(s,&local,sizeof(struct sockaddr)) < 0) {
        ftp_printf(1,"bind: %s\n",strerror(errno));
        close(s);
        return FTP_BAD;   
    }
    ...
    getnameinfo(&local, sizeof(local), name, sizeof(name), port, sizeof(port),NI_NUMERICHOST|NI_NUMERICSERV);
...
/* Receive the file into the buffer and close the data socket
   afterwards */
static int receive_file(int data_s, ftp_write_t ftp_write, void *ftp_write_priv, ftp_printf_t ftp_printf)
{
    char *buf;
    int finished = 0;
    int total_size=0;
...
static int send_file(int data_s, ftp_read_t ftp_read, void *ftp_read_priv, ftp_printf_t ftp_printf)
{
    char *buf;
    int len, rlen;
    int s;
...
static int quit(int s, char *msgbuf, unsigned msgbuflen, ftp_printf_t ftp_printf) 
{
...
int ftp_get(char * hostname,char *portftp, char * username, char * passwd, char * filename, char * buf, unsigned buf_size , ftp_printf_t ftp_printf)
{
    ...
    return ftp_get_var(hostname,portftp, username, passwd, filename, _ftp_write, &ftp_data, ftp_printf);
}

int ftp_get_var(char *hostname,char *portftp, char *username, char *passwd, char *filename, ftp_write_t ftp_write, void *ftp_write_priv, ftp_printf_t ftp_printf)
{
  ...
  s = connect_to_server(hostname,&local,portftp,ftp_printf);
  if (s < 0) {
    return (s);
  }
...
int ftpupload_RiMMS(char * hostname, char *portftp, char * username, char * passwd, char * filename, char *directory, char * buf, unsigned buf_size, ftp_printf_t ftp_printf)
{
	struct _ftp_data ftp_data;

    ftp_data.buf = buf;
    ftp_data.len = buf_size;
    return ftpupload_RiMMS_var(hostname, portftp, username, passwd, filename, directory, _ftp_read, &ftp_data, ftp_printf);
}

int ftpupload_RiMMS_var(char *hostname, char *portftp, char *username, char *passwd, char *filename, char *directory, ftp_read_t ftp_read, void *ftp_read_priv, ftp_printf_t ftp_printf)
{
  ...
  if(*directory!=0){
  ret = command("CWD",directory,s,msgbuf,sizeof(msgbuf),ftp_printf);
  if (ret < 0) {
    close(s);
    close(data_s);
    return (ret);
  ...
  if (ret != 2) {
    ftp_printf(1,"FTP: CWD failed!\n");
    close(s);
    return (FTP_BADDIRECTORY);
  }
  }
  ...
  sprintf(tmpfilename,"%s_",filename);
  /* Ask for the file */
  ret = command("STOR",tmpfilename,s,msgbuf,sizeof(msgbuf),ftp_printf);
  if (ret < 0) {
    close(s);
    close(data_s);
    return (ret);
  ...
  //RNFR from-filename
  ret = command("RNFR",tmpfilename,s,msgbuf,sizeof(msgbuf),ftp_printf);
  if (ret < 0) {
    close(s);
    close(data_s);
    return (ret);
  ...
  if (ret != 3) {
    ftp_printf(1,"FTP: RNFR failed!\n");
    close(s);
    return (FTP_RNFR);
  }
  
  //RNTO to-filename
  ret = command("RNTO",filename,s,msgbuf,sizeof(msgbuf),ftp_printf);
  if (ret < 0) {
    close(s);
    close(data_s);
    return (ret);
...
int ftp_put(char * hostname, char * portftp, char * username, char * passwd, char * filename, char * buf, unsigned buf_size, ftp_printf_t ftp_printf)
{
    struct _ftp_data ftp_data;

    ftp_data.buf = buf;
    ftp_data.len = buf_size;
    return ftp_put_var(hostname, portftp, username, passwd, filename, _ftp_read, &ftp_data, ftp_printf);
}

int ftp_put_var(char *hostname, char *portftp, char *username, char *passwd, char *filename, ftp_read_t ftp_read, void *ftp_read_priv, ftp_printf_t ftp_printf)
{
